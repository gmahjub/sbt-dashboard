...
def get_contract_data(fn, contract):

    sheet_name = contract.upper() + ' Data'
    creds = get_creds()
    client = gspread.authorize(creds)
    worksheet = client.open(fn).worksheet(sheet_name)
    rows = worksheet.get_all_records()
    df = pd.DataFrame(rows)
    # first row can be dropped since we don't have return data there
    df = df[1:]
    df = df.replace({"%": ""}, regex=True)
    cols_to_numeric = ['Open', 'High', 'Low', 'Last', 'Change', 'Settle', 'Est. Volume',
                       'Prior Day OI', 'Day Low Return', 'Day High Return', 'Day Return', 'CoD', 'Gap At',
                       'Gap Filled?', 'Day Range']
    for col in cols_to_numeric:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    df['Day Low Return'] = df['Day Low Return'] / 100.0
    df['Day High Return'] = df['Day High Return'] / 100.0
    df['Day Return'] = df['Day Return'] / 100.0
    df['Day Range'] = df['Day Range'] / 100.0
...
def get_models_list(fn, sheet_name):
...
def get_model_signal_data(fn, sheet_name):
    ...
    underlying_px_contracts = ['ES']

    # check if the last row is today's date
    df = df.replace({"%": ""}, regex=True)
    df_recent = df[pd.to_datetime(df.Date).dt.date >= pd.to_datetime('today').date()].copy(deep=True)
    df = df[pd.to_datetime(df.Date).dt.date < pd.to_datetime('today').date()]
    df = df.dropna()

    cols_to_numeric = ['Expected Return', 'Hit Rate', 'Risk', 'Reward', 'Actual Return', 'Day Low Return',
                       'Day High Return', 'Long Always', 'DYTC Model CoD', 'Within Risk @ Extrema?',
                       'Within Risk @ Settle?', 'DYTC Model Total', 'Rate Vol (AC)', 'Rate Vol (AB)', 'VIX',
                       'VIX Return', 'SmoothAC-SPX Correlation', 'Fundamental Model Total', '5-D Long Model Total',
                       '5-D Long Cumm Model Total', 'SPY(CM) Total, ES Pts', 'SPY(AH) Total, ES Pts', 'ES CoD']
    for col in cols_to_numeric:
        df[col] = pd.to_numeric(df[col], errors='coerce')

    # cols to int
    df = df.astype({'SPX Vol (BQ)': 'int'})

    df['Expected Return'] = df['Expected Return']/100.0
    df['Hit Rate'] = df['Hit Rate'] / 100.0
    df['Risk'] = df['Risk'] / 100.0
    df['Reward'] = df['Reward'] / 100.0
    df['Actual Return'] = df['Actual Return'] / 100.0
    df['Day Low Return'] = df['Day Low Return'] / 100.0
    df['Day High Return'] = df['Day High Return'] / 100.0
    create_time = datetime.now()
    df.Date = pd.to_datetime(df.Date).dt.date
    df.sort_values(by='Date', inplace=True, ascending=False)
    df_dict = split_model_signal_dataframes(df, underlying_px_contracts)
    df_dict['LatestSignalData'] = df_recent
    return df_dict, create_time


def split_model_signal_dataframes(df, price_data_contracts=None):

    df1_cols = ['Date', 'Bias', 'DYTC Units', 'Expected Return', 'Hit Rate', 'Risk', 'Reward', 'Actual Return',
                'Within Risk @ Extrema?', 'Within Risk @ Settle?']
    df2_cols = ['Date', 'Actual Return', 'Day Low Return', 'Day High Return', 'ES CoD', 'DYTC Model CoD',
                'DYTC Model Total', 'Long Always', 'Fundamental Model Total', '5-D Long Model Total',
                '5-D Long Cumm Model Total', 'SPY(CM) Total, ES Pts', 'SPY(AH) Total, ES Pts']
    df3_cols = ['Within Risk @ Extrema?', 'Within Risk @ Settle?', 'Expected Return Error']
    df4_cols = ['Distribution Size (in-sample)', 'Distribution Size (out-sample)',
                'Outsample Confirms?', 'Multiple Scenarios?']
    df5_cols = ['Date', 'Bias', 'Fundamental Model', 'Fundamental Model Leverage', 'Fundamental Model CoD',
                   'Fundamental Model Total', 'DYTC Units', '5-Day Signal, Each PPnL', '5-D Long Units',
                   '5-D Long Day P&L', '5-D Long Model Total', '5-D Long Cumm Model Total', 'ES CoD', 'DYTC Model CoD',
                   'Long Always', 'Within Risk @ Settle?', 'Within Risk @ Extrema?', 'SPY(AH) Day P&L, ES Pts',
                'SPY(AH) Total, ES Pts', 'DYTC Model Total']
    df6_cols = ['Date', 'Rate Vol (AB)', 'Rate Vol (AC)', 'Actual Return', 'Within Risk @ Settle?',
                'Within Risk @ Extrema?', 'SPX Vol (BQ)', 'VIX Return', 'SmoothAC-SPX Correlation']
    df7_cols = ['Date', 'Fundamental Model CoD', 'DYTC Model CoD', 'ES CoD', 'SPY(AH) Day P&L, ES Pts',
                '5-D Long Day P&L', '5-D Cumm Signal Day P&L']
    df8_cols = ['Date', 'Actual Return', 'Day Low Return', 'Day High Return', 'ES CoD', 'DYTC Model CoD',
                'DYTC Model Total', 'Long Always', 'Fundamental Model Total']
    df9_cols = ['Date', 'Bias', 'DYTC Units']
    expected_return_stats_cols = ['Date', 'Bias', 'DYTC Units', 'Expected Return', 'Dist Std']
    re_analysis_cols = ['Date', 'Risk Event?', 'Risk Event Time', 'Invalidate on Risk Event?', 'Comments', 'ES CoD',
                        'Bias', 'DYTC Units', 'DYTC Model CoD']

    df_dict = {'DYTC Model Expectations': df[df1_cols],
               'Realized Performance': df[df2_cols],
               'Risk Stats': df[df3_cols],
               'Miscellaneous': df[df4_cols],
               'Accuracy': df[df5_cols],
               'Rate Vol': df[df6_cols],
               'Histogram Data': df[df7_cols],
               'DYTC Realized Performance': df[df8_cols],
               'Integrity Data': df[df9_cols],
               'Risk Event Analysis': df[re_analysis_cols],
               'DYTC Px Levels': df[expected_return_stats_cols]}

    if price_data_contracts is not None:
        for pdc in price_data_contracts:
            pdc_df = get_contract_data("Prod-Dashboard Data", pdc)
            pdc_df.Date = pd.to_datetime(pdc_df.Date).dt.date
            df.Date = pd.to_datetime(df.Date).dt.date
            pdc_df = pdc_df[pdc_df.Date.isin(df.Date.values)]
            df_dict[pdc] = pdc_df
...
def get_dytc_px_levels(df_ms, neg_sigmas=0.5, pos_sigmas=0.5):
    ...
    zero_flag_osl_cnt = work_df[(work_df['Flag, OSL'] == 0) & (work_df['Pts Ret, OSL'] != 0.0)]['Pts Ret, OSL'].count()
    zero_flag_osl_mean = work_df[(work_df['Flag, OSL'] == 0) & (work_df['Pts Ret, OSL'] != 0.0)]['Pts Ret, OSL'].mean()
    ...
    zero_flag_obl_cnt = work_df[(work_df['Flag, OBL'] == 0) & (work_df['Pts Ret, OBL'] != 0.0)]['Pts Ret, OBL'].count()
    zero_flag_obl_mean = work_df[(work_df['Flag, OBL'] == 0) & (work_df['Pts Ret, OBL'] != 0.0)]['Pts Ret, OBL'].mean()
